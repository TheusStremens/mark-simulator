...
T MessageQueue<T>::receive()
{
    ...
    std::unique_lock<std::mutex> lckg(_mutex);
    _condition.wait(lckg, [this] { return !_queue.empty(); }); // pass unique lock to condition variable
...
void TrafficLight::waitForGreen()
{
    ...
    while (true)
    {
        auto lightPhase = _messageQueue.receive();
...
void TrafficLight::cycleThroughPhases()
{
    ...
    while (true)
    {
        ...
        long secondsSinceLastUpdate =
            std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now() - lastUpdate).count();
